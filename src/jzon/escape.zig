//! zix jzon string escape rules (RFC 8259 7).
//!
//! What:
...
test "zix jzon: escape leaves a clean string alone" {
...
test "zix jzon: escape spells the quote, the backslash and the named controls" {
...
test "zix jzon: escape spells an unnamed control byte as lowercase hex" {
    var buf: [32]u8 = undefined;
    var sink: Sink = .init(&buf);
...
test "zix jzon: escape agrees with std.json.Stringify byte for byte" {
...
test "zix jzon: needsEscape sees exactly what encode would spell" {
...
test "zix jzon: decode accepts the escaped solidus encode never emits" {
...
test "zix jzon: decode joins a surrogate pair into one code point" {
    var buf: [16]u8 = undefined;
    var sink: Sink = .init(&buf);
...
test "zix jzon: decode rejects an unpaired surrogate half" {
    var buf: [16]u8 = undefined;
