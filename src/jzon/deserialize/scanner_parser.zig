//! zix jzon deserialize, tokens from std's scanner.
//!
//! What:
...
test "zix jzon: scanner parser reads a whole record" {
...
test "zix jzon: scanner parser reads the same record the std path reads" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix jzon: scanner parser reports the same failures the std path reports" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix jzon: scanner parser steps over an unknown key when asked to" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix jzon: scanner parser borrows a clean string and copies an escaped one" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix jzon: scanner parser refuses a document with anything after the value" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
