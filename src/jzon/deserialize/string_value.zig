...
test "zix jzon: a borrowed clean string is the document's own bytes" {
...
test "zix jzon: a copied clean string leaves the document behind" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix jzon: an escaped string is decoded in either mode" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix jzon: a decoded string is exactly as long as its value" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix jzon: a string carrying an escape the rules do not spell is refused" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
