...
/// Usage:
/// ```zig
/// const order = try zix.jzon.deserialize(Order, arena, body, .{});
///
/// const faster = try zix.jzon.deserialize(Order, arena, body, .{
///     .strategy = .GENERATED,
///     .strings = .BORROW,
///     .unknown = .SKIP,
/// });
...
test "zix jzon: deserialize reads a record through every strategy" {
...
test "zix jzon: deserialize carries the string mode down to the path it picked" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix jzon: deserialize carries the unknown key rule down to the path it picked" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix jzon: deserialize reports every strategy's failure through one error set" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix jzon: deserialize reads a shape the generated paths refuse" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
