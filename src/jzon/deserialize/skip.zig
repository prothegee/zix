//! zix jzon deserialize, stepping over a value.
//!
//! What:
...
test "zix jzon: skip steps over each scalar form" {
...
test "zix jzon: skip steps over a whole nested value" {
...
test "zix jzon: skip steps over a value laid out with whitespace" {
    try expectLeaves("  { \"deep\" : [ 1 , 2 ] , \"b\" : true }  ,rest", "  ,rest");
    try expectLeaves("  \"padded\"  ,rest", "  ,rest");
}

test "zix jzon: skip refuses a value it cannot walk to the end of" {
...
test "zix jzon: skip refuses a malformed value the way the read paths do" {
...
test "zix jzon: skip refuses nesting past the cap" {
