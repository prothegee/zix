...
/// const faster = try zix.jzon.serialize(&buf, order, .{ .strategy = .GENERATED });
/// ```
///
/// Return:
...
test "zix jzon: serialize renders a record through every strategy" {
...
test "zix jzon: serialize returns the length of what it wrote" {
...
test "zix jzon: serialize reports a buffer the value does not fit in" {
    inline for (STRATEGIES) |strategy| {
...
test "zix jzon: serialize renders a shape the generated paths refuse" {
