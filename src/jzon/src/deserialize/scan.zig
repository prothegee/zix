...
test "jzon: both scan widths step over the same whitespace" {
...
test "jzon: both scan widths bound the same string token" {
    const documents = [_][]const u8{
        "\"\"",
        "\"plain\"",
...
test "jzon: the scan width defaults to the scalar one" {
    const shape: Shape = .{};
