...
test "zix jzon: reflect answers which enums cover every value they carry" {
    try std.testing.expect(isExhaustive(Closed));
    try std.testing.expect(!isExhaustive(Open));
}

test "zix jzon: reflect reads the default a field declares" {
    try std.testing.expectEqual(@as(?u8, 3), defaultOf(Record, 3));
}

test "zix jzon: reflect separates no default from a default of null" {
