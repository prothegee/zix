...
///
/// Note:
/// - Three comparisons and three reductions, all lane parallel. The question is
///   only whether the lane is clean, never which byte is not, so the position is
///   left to the scalar pass that follows a hit.
inline fn anyEscaped(chunk: Chunk) bool {
    ...
    return @reduce(.Or, chunk == quote) or
        @reduce(.Or, chunk == backslash) or
        @reduce(.Or, chunk < first_printable);
}
