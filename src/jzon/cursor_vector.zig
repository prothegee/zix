//! zix jzon read cursor, scanned a vector lane at a time.
//!
//! What:
...
test "zix jzon: vector skipSpace stops where the scalar skip stops" {
...
test "zix jzon: vector skipSpace steps over runs longer than one lane" {
...
test "zix jzon: vector stringSpan reads what the scalar span reads" {
    for (DOCUMENTS) |document| {
        var ours: Cursor = .init(document);
...
test "zix jzon: vector stringSpan agrees with the scalar one at every position" {
...
test "zix jzon: vector stringSpan leaves the bytes after the token unread" {
