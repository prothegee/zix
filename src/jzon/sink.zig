...
test "zix jzon: sink writes bytes, literals and runs in order" {
...
test "zix jzon: sink reserve hands back a region the caller fills" {
...
test "zix jzon: sink tail and commit take a foreign writer's length" {
...
test "zix jzon: sink reset drops what was written" {
    var buf: [8]u8 = undefined;
    var sink: Sink = .init(&buf);
...
test "zix jzon: sink refuses a write that does not fit whole" {
