...
fn UringWorker(comptime handler_fn: HandlerFn, comptime raw_fn: ?core.RawFn) type {
    return struct {
        ...
        /// constant directly) so tests can drive the overflow path with a small
        /// pool.
        ...
        /// steady connection churn never reclaims a buffer it is about to reuse.
        /// The floor keeps a small warm reserve when the worker is idle. Deriving
        /// from live_count (not a flat constant) makes the cap scale with the
        /// per-worker concurrency, which differs by how many workers split the host.
        ...
        fn releaseConn(self: *Self, conn: *UringConn) void {
            ...
            if (self.free_count >= self.idleCap()) {
                slab.releaseSlabPages(conn.buf);
