...
//!   role labels differ. They are reached through postgrez.tls, which names
...
const postgrez_tls = @import("postgrez").tls;

const key_schedule = postgrez_tls.key_schedule;
const record = postgrez_tls.record;
...
test "postgrez inproc: tls server hello echoes the session id and carries a key share" {
...
test "postgrez inproc: tls parses a client hello built by the driver" {
    const client = postgrez_tls.client;
...
test "postgrez inproc: tls rejects a hello that offers no usable cipher suite" {
...
test "postgrez inproc: tls certificate message wraps the der in a certificate list" {
    var threaded = std.Io.Threaded.init(testing.allocator, .{});
    defer threaded.deinit();

    const cert = try certificate.SelfSigned.generate(threaded.io(), "postgrez-inproc");
