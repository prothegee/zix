...
/// THE ONLY SOURCE OF TRUTH for Zig SEMVER for the postgrez build.
///
/// Note:
...
fn ensureSupportedZig() void {
    ...
    @compileError(std.fmt.comptimePrint(
        "postgrez build requires Zig 0.16.x or 0.17.x, found {d}.{d}.{d}. " ++
...
///   the container name and port: run them in separate invocations. On a
///   failed run the container stays up, the next run's pre-clean (or
///   `docker rm -f zix-postgrez-pg18`) removes it.
fn addContainerStart(b: *std.Build) *std.Build.Step.Run {
    const image_build = b.addSystemCommand(&.{
        "docker", "build", "-t", "zix-postgrez-pg18-img", "../../../containers/postgresql",
    });

    const pre_clean = b.addSystemCommand(&.{
        "sh", "-c", "docker rm -f zix-postgrez-pg18 >/dev/null 2>&1 || true",
    });
    ...
    const container_run = b.addSystemCommand(&.{
        "docker",                "run",
        "--rm",                  "-d",
        "--name",                "zix-postgrez-pg18",
        "-e",                    "POSTGRES_PASSWORD=postgrez_admin_pw",
        "-e",                    "POSTGRES_DB=postgrez_test",
        "-p",                    "127.0.0.1:54180:5432",
        "--tmpfs",               "/var/lib/postgresql",
        "zix-postgrez-pg18-img", "postgres",
        "-c",                    "fsync=off",
...
pub fn build(b: *std.Build) void {
    ...
    // Installed example binaries carry the target triple and the optimize mode
    // so per-target and per-mode builds coexist in zig-out/bin. A foreign target
    // (cross build) compiles every suite and example but skips execution and the
    // container lifecycle.
    ...
    const examples_step = b.step("examples", "Build every postgrez example into zig-out/bin");
    ...
    inline for (example_names, 0..) |name, index| {
        ...
        const example_exe = b.addExecutable(.{
            .name = b.fmt("postgrez-example-{s}-{s}-{s}", .{ name, triple, mode }),
    ...
    runner_module.addImport("postgrez", postgrez);

    const runner_exe = b.addExecutable(.{
        .name = "postgrez-runner",
