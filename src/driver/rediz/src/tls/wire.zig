...
pub const DecodeError = error{RedizTruncated};
...
pub const Reader = struct {
    ...
    pub fn readU8(self: *Reader) DecodeError!u8 {
        if (self.pos + 1 > self.buf.len) return error.RedizTruncated;
    ...
    pub fn readU16(self: *Reader) DecodeError!u16 {
        if (self.pos + 2 > self.buf.len) return error.RedizTruncated;
    ...
    pub fn readU24(self: *Reader) DecodeError!u32 {
        if (self.pos + 3 > self.buf.len) return error.RedizTruncated;
    ...
    pub fn readBytes(self: *Reader, n: usize) DecodeError![]const u8 {
        if (self.pos + n > self.buf.len) return error.RedizTruncated;
...
test "rediz tls: wire reserve + patch a nested u16 vector" {
    ...
    try std.testing.expectError(error.RedizTruncated, reader.readU8());
}
