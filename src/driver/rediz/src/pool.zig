...
//!   fully-held pool sheds immediately with error.RedizPoolExhausted), N parks
//!   up to N acquires FIFO and hands each released connection directly to
//!   the oldest waiter. Beyond N acquire sheds with error.RedizPoolBusy.
...
pub const Pool = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, io: std.Io, config: lib.Config) !Self {
        if (config.pool_size == 0) return error.RedizPoolSizeNotConfigured;
    ...
    /// - error.RedizPoolExhausted when every slot is held and parking is off
    /// - error.RedizPoolBusy when process_queue_len waiters are already parked
    ...
    pub fn acquire(self: *Self) !*Conn {
        ...
        if (self.config.process_queue_len == 0) {
            ...
            return error.RedizPoolExhausted;
        }
        if (self.waiter_count >= self.config.process_queue_len) {
            self.unlock();

            return error.RedizPoolBusy;
        }
...
test "rediz: pool init validates pool_size and starts empty" {
    ...
    try testing.expectError(error.RedizPoolSizeNotConfigured, Pool.init(testing.allocator, threaded.io(), .{
        .pool_size = 0,
    }));
...
test "rediz: pool sheds PoolBusy beyond the waiter bound" {
    ...
    try testing.expectError(error.RedizPoolBusy, pool.acquire());
