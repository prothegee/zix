//! rediz example runner: verifies every example binary against the live
//! Redis 8 container.
//!
//! Note:
...
/// Poll until the container accepts a driver connection (the image pull on
/// first start takes a while).
fn waitForServer(io: std.Io, allocator: std.mem.Allocator) !void {
    var attempt: u32 = 0;
    while (true) : (attempt += 1) {
        const conn = rediz.Conn.connect(allocator, io, .{
            .ip = IP,
            .port = PORT,
        }) catch {
