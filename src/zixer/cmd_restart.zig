//! zixer restart command: re-read one site cfg and rebind it in the daemon
...
const USAGE =
    \\usage:
    \\    zixer restart <site.cfg>
    \\
    \\re-reads the site cfg from disk and rebinds it, spawning the daemon when
    \\it is not running. A site that was not started is simply started, so a
    \\certbot deploy-hook never fails on a site that happened to be down.
    \\
;
...
///
/// Return:
/// - u8 process exit code (0 when the site runs again, 1 on any refusal)
...
test "zix zixer: cmd restart, no name shows usage" {
    ...
    const code = try run(io, arena.allocator(), &report, .{ .path = "tmp/zixer_cmd_restart_usage", .source = .ARG }, &.{}, "zixer");

    try std.testing.expectEqual(@as(u8, 1), code);
    try std.testing.expect(std.mem.indexOf(u8, report.buffered(), "zixer restart <site.cfg>") != null);
}

test "zix zixer: cmd restart, extra names show usage" {
    var threaded = std.Io.Threaded.init(std.testing.allocator, .{});
    defer threaded.deinit();
    const io = threaded.io();
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    ...
    var report_buf: [2048]u8 = undefined;
    var report = std.Io.Writer.fixed(&report_buf);
    const code = try run(io, arena.allocator(), &report, .{ .path = "tmp/zixer_cmd_restart_usage", .source = .ARG }, &.{ "a.cfg", "b.cfg" }, "zixer");

    try std.testing.expectEqual(@as(u8, 1), code);
    try std.testing.expect(std.mem.indexOf(u8, report.buffered(), "zixer restart <site.cfg>") != null);
}

test "zix zixer: cmd restart, missing root asks for zixer init before any spawn" {
    var threaded = std.Io.Threaded.init(std.testing.allocator, .{});
    defer threaded.deinit();
    const io = threaded.io();
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    ...
    var report_buf: [2048]u8 = undefined;
    var report = std.Io.Writer.fixed(&report_buf);
    const code = try run(io, arena.allocator(), &report, .{ .path = "tmp/zixer_cmd_restart_absent/root", .source = .ARG }, &.{"service_a"}, "zixer");
