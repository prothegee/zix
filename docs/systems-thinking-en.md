...
| Protocol conformance and handshake inspection | `curl -v`, and `curl --http3-only -v` for QUIC | the live behavioral oracle: `-v` narrates each step, so for HTTP/3 it pinpoints how far a partial handshake gets (Initial, ServerHello, certificate, 1-RTT, request). needs curl built with an HTTP/3 backend (ngtcp2 / nghttp3), confirm with `curl --version` |
