...
- Dukungan method HTTP QUERY di kedua engine HTTP/1 (RFC 10008):
    ...
    - Sebelum ini, `zix.Http1` mem-parse request QUERY dengan baik lalu melaporkannya sebagai `GET`, karena `enumFromString` menjawab `GET` untuk token apa pun yang tidak dikenalinya, sehingga handler tidak bisa membedakan pertanyaan yang dibawa di content dari GET biasa. `zix.Http` menolak request line-nya mentah-mentah dengan `error.InvalidRequest`, jadi ia tidak pernah sampai ke route sama sekali.
    - Perubahan perilaku: method yang tidak diimplementasikan engine kini menjawab `501 Not Implemented`. `error.UnknownMethod` yang baru hanya muncul saat request line ter-tokenisasi dan hanya method-nya yang tidak didukung, jadi baris yang memang rusak tetap menjawab `400` (RFC 9110 section 15.6.2). `Method.codeFromString` melaporkan tidak ada kecocokan sebagai null, sementara `enumFromString` mempertahankan fallback GET untuk head yang dibangun manual.
...
- Engine baru `zix.Webrtc`, sebuah peer WebRTC (ADR-067):
    ...
    - `run()` memvalidasi sebelum bind: `error.PortNotConfigured`, `error.IceCredentialsRequired`, `error.IceCredentialsInvalid`, `error.TlsRequired`, `error.UnsupportedCertificateKey` (kuncinya harus ECDSA P-256, karena satu-satunya suite DTLS 1.2 di sini adalah ECDHE-ECDSA), dan `error.DispatchModelUnsupported` di luar Linux.
