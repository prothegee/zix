...
| conn_timeout_ms | 0 | penjaga umur koneksi (ms), 0 = nonaktif (ADR-062) | eviction timer latar pada `.ASYNC`/`.POOL`/`.MIXED` | atur untuk meng-evict koneksi berumur panjang pada model blocking | koneksi diputus lebih cepat | koneksi berumur lebih panjang | no-op terdokumentasi pada `.EPOLL`/`.URING` (event loop-nya memiliki umur koneksi) |
...
- `.EPOLL` dan `.URING` khusus Linux. Di luar Linux keduanya melipat ke `.POOL`. Jalur TLS mengikuti model: `.EPOLL` / `.URING` terminasi di worker tls_mux yang multiplexed, model thread di tls_serve.
