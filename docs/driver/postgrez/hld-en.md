...
`dispatch.Line` is the reactor-less sibling for a caller that owns its own event loop (zix.Http1's `.URING` external watch): one connection, `submit` stages, the caller `flush`es once per batch (`pump` flushes too) so many requests leave in one write, `pump` reads and delivers framed replies, and the caller watches the fd for readability itself. A closed peer surfaces as `error.PostgrezConnectionClosed` from `pump`.
