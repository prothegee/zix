...
| process_queue_len | `0` | pool saja: batas acquire yang parkir, 0 = tanpa parkir | perilaku acquire saat pool penuh | setel ke jumlah worker plus margin (lihat bagian sizing) | acquire shed alih-alih parkir | lebih banyak thread parkir (blokir) alih-alih shed | 0 shed `error.PostgrezPoolExhausted` seketika, melewati batas shed `error.PostgrezPoolBusy` |
...
Biaya round trip dibayar sekali untuk seluruh batch alih-alih sekali per query. `max_pending_replies` adalah batas kedalaman: `sendRows` melewati batas shed `error.PostgrezQueueFull` sehingga producer liar tak bisa menumbuhkan send buffer tanpa batas.
...
flowchart LR
    ...
    q -->|di batas| shed1[error.PostgrezPoolBusy]
```

- 0 berarti tanpa parkir: pool penuh shed `error.PostgrezPoolExhausted` seketika. Pilih ini saat Anda ingin backpressure langsung sampai ke pemanggil.
- `N` memarkir hingga `N` acquire FIFO dan menyerahkan tiap koneksi yang di-release langsung ke waiter tertua. Melewati `N`, acquire shed `error.PostgrezPoolBusy`. Pilih ini saat macet sesaat sebaiknya menunggu bukan gagal.
