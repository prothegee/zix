...
Cleartext only, GET and POST only. Two body-framing modes: `Content-Length` and chunked transfer-encoding. Chunked support was not in the original scope, it was added after live validation against a real node-exporter: node-exporter sends `Transfer-Encoding: chunked` with no `Content-Length` for its `/metrics` response, so a client that only understood `Content-Length` failed against a real target from the first request. `connect_timeout_ms` is accepted on every config for API-shape parity but not yet enforced: this Zig version's `std.Io.Threaded.netConnectIpPosix` panics on a timeout request rather than erroring.
