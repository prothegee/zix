...
**CT (Compile Time):**<br>
Zix pushes work to `comptime` whenever the input is fixed at build time. Route tables are partitioned and dispatch is specialized per route set (`Router(comptime routes)`, `Grpc.Router(comptime RouterType)` threaded through gRPC's own dispatch models since `is_server_streaming` must stay engine-visible before dispatch), and version differences are gated out with `ZIG_SEMVER` (`comptime ZIG_SEMVER.MINOR == 16`) so only the active branch is compiled. The trade is build-time work and binary size for zero runtime branching on the hot path.
