...
| Conformance protokol dan inspeksi handshake | `curl -v`, dan `curl --http3-only -v` untuk QUIC | live behavioral oracle: `-v` menarasikan tiap langkah, jadi untuk HTTP/3 ia menunjukkan sejauh mana handshake parsial sampai (Initial, ServerHello, certificate, 1-RTT, request). butuh curl yang dibangun dengan backend HTTP/3 (ngtcp2 / nghttp3), cek dengan `curl --version` |
