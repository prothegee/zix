...
| Atribusi simbol (function mana yang stall) | `perf record -e L1-dcache-load-misses -c 2000 -p <pid>` lalu `perf report --stdio` / `perf annotate` | butuh sudo, dan sandbox agent mematikan `perf record` (signal)|
