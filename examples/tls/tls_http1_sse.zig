...
// curl -N -k https://localhost:9073/events
//
// Begins the SSE stream, then emits "tick N" once per second, each event encrypted as one TLS
// record. The loop ends when a write fails, which is how a disconnected client is seen.
...
fn eventsHandler(_: *zix.Http1.Request, res: *zix.Http1.Response, ctx: *zix.Http1.Context) !void {
    // sendStream() detaches the buffered capture so each event flushes immediately over TLS. An
