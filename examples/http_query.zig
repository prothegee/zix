...
///
/// Param:
/// content_type - zix.Http.ContentType (already stripped of its parameters)
///
/// Return:
/// - bool
fn acceptsQueryType(content_type: zix.Http.ContentType) bool {
...
///
/// Param:
/// content_type - zix.Http.ContentType (the type the question is written in)
...
fn rowsMatching(content_type: zix.Http.ContentType, query: []const u8) ?usize {
...
fn sendUnsupportedType(res: *zix.Http.Response) !void {
...
// curl usage: curl -X QUERY "http://localhost:9080/search" -H "Content-Type: application/sql" -d "SELECT name FROM users"
fn searchHandler(req: *zix.Http.Request, res: *zix.Http.Response, _: *zix.Http.Context) !void {
    if (req.method() != .QUERY) {
    ...
    const content_type = zix.Http.Content.typeFromHeader(header_value) orelse {
...
// curl usage: curl -X GET "http://localhost:9080/status"
fn statusHandler(_: *zix.Http.Request, res: *zix.Http.Response, _: *zix.Http.Context) !void {
