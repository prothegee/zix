# localbench: http2-uring

zix.Http2 on the `.URING` dispatch model: one SO_REUSEPORT listener plus io_uring completion ring per worker, shared-nothing across cores.
...
```bash
./scripts/localbench-build.sh http2-uring
