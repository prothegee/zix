//! localbench: zix
//!
//! zix.Http1 (.ASYNC), Router-only: every request goes through the engine's
//! parser and the comptime Router, one handler module per route
//! (src/handlers/). TLS rides tls_port (dual listener, one accept thread).
//! /static is served by the engine from public_dir. async-db and crud run
//! over lanes (dbpg.zig): each worker thread owns a pipelined postgres
...
pub fn main(process: std.process.Init) !void {
    ...
    // public_dir below.
    // DB endpoints: lanes open lazily per worker once DATABASE_URL is present
    // (non-DB profiles open nothing, DB routes answer 503).
    dbpg.init(process);
    dbpg.start();
    ...
    // workers is omitted on purpose: .ASYNC always runs exactly one accept
    // thread and dispatches each connection through io.async().
