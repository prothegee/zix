# localbench: http2-async

zix.Http2 on the `.ASYNC` dispatch model: one accept thread handing each connection to a task.
...
```bash
./scripts/localbench-build.sh http2-async
./scripts/localbench-validate.sh http2-async
./scripts/localbench-run.sh http2-async
```
