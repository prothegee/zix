# localbench: http1-ws-epoll

zix.Http1 WebSocket on the `.EPOLL` dispatch model: one SO_REUSEPORT listener plus epoll set per worker, shared-nothing across cores.
...
```bash
./scripts/localbench-build.sh http1-ws-epoll
./scripts/localbench-validate.sh http1-ws-epoll
./scripts/localbench-run.sh http1-ws-epoll
```
