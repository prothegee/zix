# localbench: http2-grpc-epoll

zix.Grpc over h2c on the `.EPOLL` dispatch model: one SO_REUSEPORT listener plus epoll set per worker, shared-nothing across cores. Built on zix.Http2,
...
```bash
./scripts/localbench-build.sh http2-grpc-epoll
./scripts/localbench-validate.sh http2-grpc-epoll
./scripts/localbench-run.sh http2-grpc-epoll
```
