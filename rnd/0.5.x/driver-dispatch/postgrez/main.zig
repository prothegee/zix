//! postgrez transport PoC: drive the real HttpArena CRUD workload against the
//! PG 18 bench container under three dispatch models and compare raw DB
//! throughput. No HTTP anywhere. std only.
//!
//! ASYNC: K blocking connections, one round trip in flight each (io.async).
//! EPOLL: one thread, K non-blocking connections, W queries pipelined each.
...
/// One profile per operation, so insert vs select vs update vs transaction each
/// gets its own throughput row.
/// GET: indexed select by primary key (transport-bound, postgres is cheap).
/// SCAN: range select on the un-indexed price column (seq scan, DB-bound).
/// INSERT: upsert on conflict. UPDATE: update by id. TRANSACTION: BEGIN, update, COMMIT.
const Profile = enum { GET, SCAN, INSERT, UPDATE, TRANSACTION };
...
/// Workload-only measurement (connection setup excluded), so the table isolates
/// the transport, not the SCRAM handshake cost. cpu_ns is process CPU time (all
/// threads) consumed during the workload, so cpu_ns / ns is the cores used.
const Result = struct { completed: u64, ns: u64, cpu_ns: u64 };

// --------------------------------------------------------- //
// workload

fn renderQuery(arena: std.mem.Allocator, rng: std.Random, profile: Profile) []const u8 {
    ...
    return switch (profile) {
        .GET => std.fmt.allocPrint(arena, "SELECT " ++ cols ++ " FROM items WHERE id = {d}", .{id}),
        .SCAN => std.fmt.allocPrint(arena, "SELECT " ++ cols ++ " FROM items WHERE price BETWEEN {d} AND {d} LIMIT 20", .{ price, price + 200 }),
        .INSERT => std.fmt.allocPrint(arena, "INSERT INTO items (" ++ cols ++ ") VALUES ({d},'item{d}','{s}',{d},{d},true,'[]',0,0) ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name,category=EXCLUDED.category,price=EXCLUDED.price,quantity=EXCLUDED.quantity", .{ id, id, cat, price, qty }),
        .UPDATE => std.fmt.allocPrint(arena, "UPDATE items SET name='item{d}',category='{s}',price={d},quantity={d} WHERE id={d}", .{ id, cat, price, qty, id }),
        .TRANSACTION => std.fmt.allocPrint(arena, "BEGIN; UPDATE items SET name='item{d}',category='{s}',price={d},quantity={d} WHERE id={d}; COMMIT", .{ id, cat, price, qty, id }),
    } catch unreachable;
}
...
fn buildWorkload(arena: std.mem.Allocator, n: usize, profile: Profile) [][]const u8 {
    ...
    const queries = arena.alloc([]const u8, n) catch unreachable;
    for (queries) |*slot| slot.* = renderQuery(arena, rng, profile);
...
fn asyncConn(ctx: AsyncCtx) void {
    ...
    while (nowNs() < ctx.deadline) {
        const idx = ctx.next.fetchAdd(1, .monotonic) % ctx.queries.len;

        const qlen = pg.encodeQuery(ctx.queries[idx], &qbuf);
        pg.writeAll(ctx.fd, qbuf[0..qlen]) catch break;

        var acc: usize = 0;
        var got: bool = false;
        while (!got) {
            const n = pg.readNb(ctx.fd, rbuf[acc..]);
            if (n == 0) break;

            acc += n;
            const result = pg.scan(rbuf[0..acc]);
            if (result.ready >= 1) {
                local += 1;
                got = true;
                break;
...
fn runAsync(gpa: std.mem.Allocator, queries: []const []const u8, conns: usize) !Result {
    const fds = try gpa.alloc(Fd, conns);
    defer gpa.free(fds);

    var opened: usize = 0;
    errdefer for (fds[0..opened]) |fd| pg.close(fd);
    while (opened < conns) : (opened += 1) fds[opened] = try pg.connectAuth(PORT, USER, PASS, DB);
    defer for (fds) |fd| pg.close(fd);
    ...
    for (futures, fds) |*future, fd| future.* = io.async(asyncConn, .{AsyncCtx{ .fd = fd, .queries = queries, .deadline = deadline, .next = &next, .completed = &completed }});
...
const Conn = struct {
    ...
    fn fill(self: *Conn, queries: []const []const u8, next: *usize) void {
        if (self.out_len != 0) return;

        while (self.inflight < WINDOW) {
            const sql = queries[next.* % queries.len];
            if (self.out_len + sql.len + 6 > self.out.len) break;

            self.out_len += pg.encodeQuery(sql, self.out[self.out_len..]);
            self.inflight += 1;
            next.* += 1;
        }
    }
    ...
    fn ingest(self: *Conn, n: usize, completed: *u64) bool {
        ...
        const result = pg.scan(self.in[0..self.in_len]);
...
fn openConns(gpa: std.mem.Allocator, conns: usize) ![]Conn {
    ...
    for (table, 0..) |*conn, idx| {
        conn.* = .{ .fd = pg.connectAuth(PORT, USER, PASS, DB) catch {
            for (table[0..idx]) |*done| pg.close(done.fd);
            gpa.free(table);

            return error.ConnectFailed;
        } };
        pg.setNonBlock(conn.fd);
    }
...
fn runEpoll(gpa: std.mem.Allocator, queries: []const []const u8, conns: usize) !Result {
    const table = try openConns(gpa, conns);
    defer {
        for (table) |*conn| pg.close(conn.fd);
        gpa.free(table);
    }
    ...
    while (nowNs() < deadline) {
        ...
        for (events[0..count]) |event| {
            ...
            if (event.events & linux.EPOLL.OUT != 0) {
                conn.out_sent += pg.writeNb(conn.fd, conn.out[conn.out_sent..conn.out_len]);
            ...
            if (event.events & linux.EPOLL.IN != 0) {
                const n = pg.readNb(conn.fd, conn.in[conn.in_len..]);
                if (n > 0) {
...
fn uringPump(ring: *linux.IoUring, conn: *Conn, idx: usize, queries: []const []const u8, next: *usize, send_armed: *bool) void {
    conn.fill(queries, next);
...
fn runUring(gpa: std.mem.Allocator, queries: []const []const u8, conns: usize) !Result {
    const table = try openConns(gpa, conns);
    defer {
        for (table) |*conn| pg.close(conn.fd);
        gpa.free(table);
    ...
    for (table, 0..) |*conn, idx| {
        ...
        uringPump(&ring, conn, idx, queries, &next, &send_armed[idx]);
    }
...
fn runModel(gpa: std.mem.Allocator, model: Model, queries: []const []const u8, conns: usize) !Result {
    return switch (model) {
        .ASYNC => runAsync(gpa, queries, conns),
        .EPOLL => runEpoll(gpa, queries, conns),
        .URING => runUring(gpa, queries, conns),
    };
}
...
/// which includes postgres and matches what top shows.
...
fn bench(gpa: std.mem.Allocator, model: Model, profile: Profile, queries: []const []const u8, conns: usize) !void {
    resetRssPeak();
    const machine0 = machineCpu();
    const result = try runModel(gpa, model, queries, conns);
    ...
    // whole-machine busy percent over the run, includes postgres, matches top
...
pub fn main() !void {
    ...
    line("postgrez transport PoC: {d}s per (model,op), K={d} conns, window={d}, pool={d}\n", .{ seconds, conns, WINDOW, POOL });
    line("  reqs = completed in {d}s (duration-based, so faster model finishes more).\n", .{seconds});
    line("  drvCPU% = driver process, 100% = one core. machCPU% = whole box (includes postgres, matches top).\n", .{});
    line("  {s:<6} {s:<12} {s:>10} {s:>10} {s:>7} {s:>8} {s:>7}\n", .{ "model", "op", "reqs", "req/s", "drvCPU%", "machCPU%", "memMB" });

    for ([_]Profile{ .GET, .SCAN, .INSERT, .UPDATE, .TRANSACTION }) |profile| {
        const queries = buildWorkload(arena, POOL, profile);
        for ([_]Model{ .ASYNC, .EPOLL, .URING }) |model| {
            try bench(gpa, model, profile, queries, conns);
        }
    }
}
