...
/// Warm idle-pool cap (A2). A closed connection is pooled with its recv and send
/// buffers intact so the next accept reuses the allocation with no allocator call,
/// which keeps the connection-churn fast path allocation-free. Past this many
/// pooled connections, the buffers of further closes have their pages returned to
/// the OS with MADV_DONTNEED, so resident memory tracks the live working set
/// instead of the lifetime high-water of concurrent connections. The struct and
/// its virtual allocation stay pooled, so reuse is still allocation-free: only the
/// physical pages are reclaimed, and the next recv first-touch faults them back.
/// This is the EPOLL release-on-close trick (slab.releaseSlabPages) bounded by a
/// cap so steady churn never takes the madvise or the re-fault.
///
/// Note:
/// - This is the throughput-gate tuning knob. Too low and churn re-faults every
///   reuse (a risk on the limited-conn cell), too high and idle memory is not
///   reclaimed. The value is provisional pending a 64-core gated run.
const URING_IDLE_POOL_WARM_CAP: usize = 256;
...
fn UringWorker(comptime handler_fn: HandlerFn, comptime raw_fn: ?core.RawFn) type {
    return struct {
        ...
        free_list: ?*UringConn,
        /// Number of connections currently parked on free_list. Tracked so the
        /// release path can tell a warm pooled connection (kept resident for a
        /// fast reuse) from an overflow one (pages returned to the OS). The single
        /// worker thread owns it, so no synchronization is needed.
        free_count: usize = 0,
        /// Warm idle-pool cap. See URING_IDLE_POOL_WARM_CAP.
        free_cap: usize = URING_IDLE_POOL_WARM_CAP,
        ...
        /// Return a closed connection to the idle pool (A2). Two reclaims keep
        /// pooled memory tracking the live working set instead of the lifetime
        /// high-water, both off the parse and handler hot path.
        ///
        /// Note:
        ...
        /// - Past the warm cap the connection's buffer pages are returned to the
        ///   OS with MADV_DONTNEED (slab.releaseSlabPages). The struct and its
        ///   virtual allocation still park on the list, so the next accept reuses
        ///   them with no allocator call, only the physical pages go back and the
        ///   next recv first-touch faults a fresh zero page. The accept path
        ///   overwrites buf and send_buf before reading, so the zeroing is safe.
        fn releaseConn(self: *Self, conn: *UringConn) void {
            ...
            if (self.free_count >= self.free_cap) {
                slab.releaseSlabPages(conn.buf);
                slab.releaseSlabPages(conn.send_buf);
            }
...
test "zix http1: URING releaseConn pools warm under cap, acquireConn reuses LIFO" {
    ...
    // LIFO: the most recently released connection is reused first, and the count
    // tracks the pool so the release path can spot an overflow.
...
test "zix http1: URING releaseConn shrinks a grown send_buf back to the base size" {
    ...
    // response. recv_buf stays small so no madvise is involved (cap not reached).
    const conn = try gpa.create(UringConn);
...
test "zix http1: URING releaseConn past the cap returns buffer pages to the OS" {
    ...
    const conn = try page.create(UringConn);
    conn.* = .{ .fd = 13, .gen = 1, .buf = try page.alloc(u8, 4096), .filled = 0, .send_buf = try page.alloc(u8, 4096), .staged = 0, .inflight = 0, .closing = false };
    defer {
        page.free(conn.buf);
        page.free(conn.send_buf);
        page.destroy(conn);
    }

    @memset(conn.buf, 0xAA);
    @memset(conn.send_buf, 0xAA);

    // free_count (0) is already at the cap (0), so this release is an overflow:
    // the buffer pages go back and read as zero on the next touch, while the
    // connection still parks on the pool for an allocation-free reuse.
    worker.releaseConn(conn);

    try std.testing.expectEqual(@as(u8, 0), conn.buf[0]);
    try std.testing.expectEqual(@as(u8, 0), conn.send_buf[0]);
    try std.testing.expectEqual(@as(?*UringConn, conn), worker.free_list);
    try std.testing.expectEqual(@as(usize, 1), worker.free_count);
}
