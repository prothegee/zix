//! TLS 1.2 record layer PoC (RFC 5246 6.2 + RFC 5288 GCM): AES-128-GCM protect / deprotect.
//! De-risks the 1.2-specific framing, which differs from 1.3: a 12-byte nonce = 4-byte salt (the
//! implicit write_IV from the key_block) ++ 8-byte explicit nonce sent on the wire, a 13-byte AAD
//! (seq ++ type ++ version ++ plaintext_len), and a record body of explicit_nonce ++ ciphertext ++
//! tag. The AEAD itself is std.crypto (NIST-tested), so the test pins the framing bytes + a NIST
//! GCM known-answer vector for the primitive. Layer R12 of tls12-plan.md.
//!
//! Run: `zig test rnd/0.5.x/tls12_record_poc.zig`

const std = @import("std");

const Aes128Gcm = std.crypto.aead.aes_gcm.Aes128Gcm;

const salt_len: usize = 4; // implicit part of the nonce (client/server_write_IV for GCM)
const explicit_nonce_len: usize = 8; // sent on the wire, zix uses the 64-bit seq (RFC 5288 3)
const tag_len: usize = Aes128Gcm.tag_length; // 16
const aad_len: usize = 13; // seq(8) + type(1) + version(2) + length(2)
const version_tls_1_2: u16 = 0x0303;

/// Build the 13-byte TLS 1.2 AEAD additional data (RFC 5246 6.2.3.3): seq_num ++ type ++ version ++
/// plaintext length, all big-endian.
...
fn protect(out: []u8, plaintext: []const u8, content_type: u8, key: [16]u8, salt: [salt_len]u8, seq: u64) []const u8 {
...
/// Decrypt one record back into `out`, verifying the tag. seq is the expected receive sequence.
fn deprotect(out: []u8, record: []const u8, key: [16]u8, salt: [salt_len]u8, seq: u64) ![]const u8 {
    ...
    try Aes128Gcm.decrypt(out[0..ct.len], ct, tag, &aad, nonce, key);
...
test "tls12 record: AAD byte layout (RFC 5246 6.2.3.3)" {
    const aad = buildAad(0x0102030405060708, 23, 0x0014); // application_data(23), len 20
    try std.testing.expectEqualSlices(u8, &[_]u8{ 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 23, 0x03, 0x03, 0x00, 0x14 }, &aad);
}

test "tls12 record: protect -> deprotect round trip + wire layout" {
    ...
    try std.testing.expectEqual(@as(u8, 23), rec[0]); // type
    try std.testing.expectEqual(version_tls_1_2, std.mem.readInt(u16, rec[1..3], .big));
    try std.testing.expectEqual(@as(u16, explicit_nonce_len + plaintext.len + tag_len), std.mem.readInt(u16, rec[3..5], .big));
    // explicit nonce on the wire is the seq (big-endian).
...
test "tls12 record: tampered tag fails auth" {
    var key: [16]u8 = undefined;
    @memset(&key, 0x11);
