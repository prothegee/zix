...
//!
//! Run: zig run rnd/http_poc_model_2_pool.zig
//! Bench: wrk -c100 -t1 -d10s http://127.0.0.1:9101/
...
// --------------------------------------------------------- //
// ConnQueue: shared work queue between accept threads and pool threads.
...
pub fn main() !void {
    ...
    std.debug.print("poc model 2 (pool): {s}:{d} ({d} accept, {d} pool)\n", .{
        IP, PORT, worker_count, pool_size,
    });
