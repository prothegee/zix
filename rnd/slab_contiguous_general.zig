//! Slab memory pattern (the zix EPOLL ConnTable) built on the general-purpose
//! smp_allocator, the allocator zix uses in production.
//!
//! The numbers come out the same as the arena variant: demand-paging and the
//! @memset cost depend on which pages you touch, not on the allocator. What the
//! allocator changes is the free model (here, each block is freed individually).
//!
//! Run in a release mode: zig run -OReleaseFast rnd/slab_contiguous_general.zig
...
pub fn main() !void {
    ...
    // A global, multi-threaded general allocator. No init, but unlike the arena
    // every block must be freed on its own.
    const a = std.heap.smp_allocator;

    p("rss at start:                  {d:.1} MiB\n", .{mib(residentBytes())});

    // A large alloc goes to its own mmap, so each block is one contiguous run of
    // address space (slab[i] and slab[i+1] are neighbours).
