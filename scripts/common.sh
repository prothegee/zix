...
#   build-all-targets family) the ZIG_BIN/target-filter parsing, one-leg
#   build, and fan-out matrix runner.
set -euo pipefail
...
# given, prints usage and exits on -h/--help or an unknown target.
parse_target_filter() {
    local zig_bin="${1:-zig}"
