...
test "zix edge: statusLevel 2xx boundary (200 -> INFO)" {
    const Logger = zix.Logger.Logger;
    _ = Logger;
    ...
    var logger = try zix.Logger.Logger.init(std.testing.allocator, .{ .save_min_level = .DEBUG });
    defer logger.deinit();
...
test "zix edge: init with empty save_path, file_fd stays invalid" {
    var logger = try zix.Logger.Logger.init(std.testing.allocator, .{ .save_path = "" });
    defer logger.deinit();
    try std.testing.expectEqual(@as(std.posix.fd_t, -1), logger.file_fd);
}
