...
test "zix edge: addHeader, CR in name returns InvalidHeaderName" {
    ...
    defer arena.deinit();
    var res = zix.Http.Response.init(undefined, undefined, arena.allocator(), 32);
...
test "zix edge: addHeader, buffer grows from 4 to 5 on the 5th header" {
    ...
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var res = zix.Http.Response.init(undefined, undefined, arena.allocator(), 5);
...
test "zix edge: addHeader, max_headers=1 rejects second header without growth" {
    ...
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var res = zix.Http.Response.init(undefined, undefined, arena.allocator(), 1);
