...
pub fn fetch(io: std.Io, server_ip: []const u8, server_port: u16, path: []const u8, body_out: []u8) ![]const u8 {
    ...
    var app_keys: keyschedule.AppKeys = undefined;
    var server_scid: [20]u8 = undefined;
    var server_scid_len: usize = 0;
    var have_sh = false;
    var have_app = false;
    ...
    while ((!have_sh or !have_app) and attempts < 16) : (attempts += 1) {
        ...
        if (hdr.packet_type == 0 and !have_sh) {
            ...
            @memcpy(server_scid[0..hdr.scid.len], hdr.scid);
            server_scid_len = hdr.scid.len;
            have_sh = true;
            ...
            app_keys = keyschedule.applicationKeys(hs_keys.handshake_secret, transcript.current());
            have_app = true;
        }
    }
    ...
    // 1-RTT request on stream 0: HTTP/3 HEADERS frame with QPACK :method GET + :path literal.
    ...
    rp = 1;
    rp += varint.write(req_payload[rp..], 0); // stream id 0
    rp += varint.write(req_payload[rp..], cl); // data length
    @memcpy(req_payload[rp..][0..cl], content[0..cl]);
    rp += cl;

    var req_pkt: [1200]u8 = undefined;
    const request = try protection.sealShort(&req_pkt, app_keys.client, server_scid[0..server_scid_len], 0, req_payload[0..rp]);
    try sock.send(io, &server, request);

    // Receive and decrypt the 1-RTT response, return its body.
    attempts = 0;
