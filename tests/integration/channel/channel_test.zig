...
test "zix integration: Channel(u32), init allocates requested capacity" {
    ...
    defer arena.deinit();
    var channel = try zix.Channel(u32).init(arena.allocator(), 8);
    defer channel.deinit();

    try std.testing.expectEqual(@as(usize, 8), channel.buf.len);
...
test "zix integration: Channel([]const u8), slice element type compiles" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var channel = try zix.Channel([]const u8).init(arena.allocator(), 4);
    defer channel.deinit();

    try std.testing.expectEqual(@as(usize, 4), channel.buf.len);
}
...
test "zix integration: Channel(struct), struct element type compiles" {
    ...
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var channel = try zix.Channel(Msg).init(arena.allocator(), 2);
    defer channel.deinit();

    try std.testing.expectEqual(@as(usize, 2), channel.buf.len);
}
...
test "zix integration: Channel(u32), send and recv round-trip" {
    ...
    const value = try channel.recv(io);
...
test "zix integration: Channel(u32), drain remaining items after close" {
    ...
    try channel.send(io, 10);
    try channel.send(io, 20);
    channel.close(io);
