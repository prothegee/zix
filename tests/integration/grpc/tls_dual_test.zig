//! Integration tests: Grpc dual listener (config.tls_port), h2c + gRPC-over-TLS from ONE worker fleet.
...
const Routes = [_]zix.Grpc.Route{
    .{ .path = "/dual.Dual/Hello", .handler = helloHandler },
};

const ServeArgs = struct {
    port: u16,
    tls_port: u16,
    dispatch_model: zix.Grpc.DispatchModel,
};
...
/// gRPC-over-TLS smoke exchange on tls_port: ALPN h2 handshake, preface + SETTINGS as application
/// data, a SETTINGS frame back inside the first record.
fn expectGrpcTlsSettings(io: std.Io, tls_port: u16) !void {
    ...
    const started = try zix.Tls.Client.start(.{
        .client_random = @splat(0x41),
        .ephemeral_secret = @splat(0x72),
        .alpn = &.{.H2},
    }, &ch_buf);
...
test "zix integration: Grpc dual listener EPOLL serves gRPC TLS on tls_port" {
    ...
    try startServersOnce();
    try expectGrpcTlsSettings(threaded.io(), EPOLL_TLS_PORT);
}
...
test "zix integration: Grpc dual listener URING serves gRPC TLS on-ring on tls_port" {
    ...
    try startServersOnce();
    try expectGrpcTlsSettings(threaded.io(), URING_TLS_PORT);
}
