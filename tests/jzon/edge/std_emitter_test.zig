//! Edge tests: zix.jzon.std_emitter at the boundaries.
...
test "zix edge: std emitter fills an exact-fit buffer" {
...
test "zix edge: std emitter refuses a buffer one byte short and writes nothing" {
...
test "zix edge: std emitter leaves an earlier write in place when its own render fails" {
...
test "zix edge: std emitter refuses an empty buffer" {
...
test "zix edge: std emitter renders the shapes with nothing in them" {
    const Empty = struct {};
...
test "zix edge: std emitter renders the extremes of the integer widths" {
...
test "zix edge: std emitter renders a value nested several levels deep" {
