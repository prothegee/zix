//! Edge tests: zix.jzon.deserialize at the boundaries.
...
fn expectRefused(comptime T: type, src: []const u8, failure: anyerror) !void {
    ...
    inline for (STRATEGIES) |strategy| {
        try std.testing.expectError(
            failure,
            zix.jzon.deserialize(T, arena.allocator(), src, .{ .strategy = strategy }),
        );
    }
}

test "zix edge: every deserialize strategy reports a document that ends early" {
...
test "zix edge: every deserialize strategy reports a document carrying more than one value" {
...
test "zix edge: every deserialize strategy reports a value the field cannot take" {
...
test "zix edge: every deserialize strategy reports an object owing a field" {
    try expectRefused(Pair, "{}", error.MissingField);
    try expectRefused(Pair, "{\"id\":1}", error.MissingField);
}

test "zix edge: every deserialize strategy reports a key the type does not declare" {
    try expectRefused(Pair, "{\"id\":1,\"name\":\"x\",\"gone\":2}", error.UnknownField);
}

test "zix edge: every deserialize strategy reports a name no enum tag carries" {
    try expectRefused(Status, "\"GONE\"", error.UnknownEnumValue);
    try expectRefused(Status, "\"pending\"", error.UnknownEnumValue);
}

test "zix edge: every deserialize strategy reads the empty string, object and array" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
    ...
    inline for (STRATEGIES) |strategy| {
        const empty = try zix.jzon.deserialize(Holder, allocator, "{\"text\":\"\",\"items\":[]}", .{
            .strategy = strategy,
        });
        ...
        _ = try zix.jzon.deserialize(struct {}, allocator, "{}", .{ .strategy = strategy });
    }
}

test "zix edge: every deserialize strategy reads the extremes of a width" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
    ...
    inline for (STRATEGIES) |strategy| {
        ...
        try std.testing.expectEqual(
            ...
            try zix.jzon.deserialize(u64, allocator, "18446744073709551615", options),
        );
        try std.testing.expectEqual(
            @as(i64, std.math.minInt(i64)),
            try zix.jzon.deserialize(i64, allocator, "-9223372036854775808", options),
        );
    }
}

test "zix edge: a generated strategy refuses an array of bytes as a string" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
    ...
    inline for (GENERATED) |strategy| {
        try std.testing.expectError(
            error.Unexpected,
            zix.jzon.deserialize([]const u8, allocator, "[104,105]", .{ .strategy = strategy }),
        );
    }

    try std.testing.expectEqualStrings("hi", try zix.jzon.deserialize([]const u8, allocator, "[104,105]", .{}));
}

test "zix edge: a generated strategy refuses a signed zero in an unsigned field" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
    ...
    inline for (GENERATED) |strategy| {
        try std.testing.expectError(
            error.BadNumber,
            zix.jzon.deserialize(Pair, allocator, src, .{ .strategy = strategy }),
        );
    }

    const theirs = try zix.jzon.deserialize(Pair, allocator, src, .{});
    try std.testing.expectEqual(@as(u8, 0), theirs.id);
}

test "zix edge: only the strategy that decodes an escape itself can name a bad one" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
    ...
    inline for ([_]Strategy{ .GENERATED, .GENERATED_VECTOR }) |strategy| {
        try std.testing.expectError(
            error.BadEscape,
            zix.jzon.deserialize([]const u8, allocator, src, .{ .strategy = strategy }),
        );
    }
...
test "zix edge: an omitted optional without a declared default is still owed" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
    ...
    inline for (STRATEGIES) |strategy| {
        try std.testing.expectError(
            error.MissingField,
            zix.jzon.deserialize(Owed, arena.allocator(), "{\"id\":1}", .{ .strategy = strategy }),
        );
    }
}

test "zix edge: every deserialize strategy reports an allocator with nothing left" {
    var failing: std.testing.FailingAllocator = .init(std.testing.allocator, .{ .fail_index = 0 });

    inline for (STRATEGIES) |strategy| {
        try std.testing.expectError(
            error.OutOfMemory,
            zix.jzon.deserialize(Pair, failing.allocator(), "{\"id\":1,\"name\":\"x\"}", .{ .strategy = strategy }),
        );
    }
}
