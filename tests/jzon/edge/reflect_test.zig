//! Edge tests: zix.jzon.reflect at the boundaries.
...
test "zix edge: reflect calls a non-exhaustive enum what it is" {
    try std.testing.expect(!reflect.isExhaustive(Open));
    try std.testing.expect(!reflect.isExhaustive(enum(u2) { A, _ }));
}

test "zix edge: reflect separates a default of null from no default" {
...
test "zix edge: reflect reads a default that is a whole struct" {
...
test "zix edge: reflect reads a default that is an empty slice" {
    const tags = reflect.defaultOf(Boundary, 2).?;

    try std.testing.expectEqual(@as(usize, 0), tags.len);
}

test "zix edge: reflect reads a default whose value is zero" {
...
test "zix edge: reflect walks a struct that declares no fields" {
