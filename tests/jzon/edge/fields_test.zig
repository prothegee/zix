//! Edge tests: zix.jzon.fields at the boundaries.
...
test "zix edge: seen refuses the same field twice" {
...
test "zix edge: seen over a target with no fields fills nothing and fails at nothing" {
    const Empty = struct {};
...
test "zix edge: seen owes an optional that declares no default" {
...
test "zix edge: seen gives an optional with a default its null" {
...
test "zix edge: seen fills the fields ahead of the one it cannot answer for" {
...
test "zix edge: two trackers over the same target do not share their marks" {
