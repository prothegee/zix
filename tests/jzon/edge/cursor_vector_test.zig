//! Edge tests: zix.jzon.cursor_vector at the boundaries.
...
test "zix edge: the lane scan reads a token whose quote lands on a lane boundary" {
...
test "zix edge: the lane scan reads an escape split across a lane boundary" {
...
test "zix edge: the lane scan reports a token the document ends inside" {
...
test "zix edge: the lane scan refuses a raw control byte inside a token" {
...
test "zix edge: the lane scan refuses a token that does not open with a quote" {
...
test "zix edge: the lane scan skips whitespace on a document shorter than a lane" {
...
test "zix edge: the lane scan skips a document that is whitespace to its end" {
...
test "zix edge: the lane scan stops on a byte the whitespace set does not hold" {
