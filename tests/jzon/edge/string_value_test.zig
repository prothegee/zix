//! Edge tests: zix.jzon.string_value at the boundaries.
...
const StringSpan = zix.jzon.StringSpan;
const string_value = zix.jzon.string_value;

test "zix edge: an empty token comes back empty in either mode" {
...
test "zix edge: a token that is nothing but escapes decodes to its value alone" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix edge: an escape the rules do not spell is refused" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix edge: a token flagged escaped that holds none comes back unchanged" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix edge: a copy reports an allocator with nothing left" {
...
test "zix edge: a borrow of a clean token asks the allocator for nothing" {
    var failing: std.testing.FailingAllocator = .init(std.testing.allocator, .{ .fail_index = 0 });
...
test "zix edge: a token as long as its decoded value keeps every byte" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
