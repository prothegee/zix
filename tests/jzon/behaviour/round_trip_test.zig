...
test "zix behaviour: every write path renders an order every read path reads back" {
    try expectRoundTrips(ORDER);
}

test "zix behaviour: every pair round trips an order with an unset optional" {
    var order = ORDER;
    order.note = null;
...
test "zix behaviour: every pair round trips an order carrying escapes" {
    var order = ORDER;
...
test "zix behaviour: every pair round trips an order with empty slices" {
    var order = ORDER;
    order.tags = &.{};
    order.lines = &.{};
...
test "zix behaviour: every pair round trips the extremes of the number widths" {
    var order = ORDER;
...
test "zix behaviour: every write path renders the same bytes" {
