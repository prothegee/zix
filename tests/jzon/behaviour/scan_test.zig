//! Behaviour tests: zix.jzon.scan, the choice between the two read widths.
...
test "zix behaviour: the scan width defaults to the byte at a time one" {
    const shape: Shape = .{};

    try std.testing.expectEqual(scan.ScanPath.SCALAR, shape.scan);
}

test "zix behaviour: both widths leave the cursor on the same byte" {
...
test "zix behaviour: both widths bound the same string token" {
    const documents = [_][]const u8{
        "\"\"",
        "\"plain\"",
...
test "zix behaviour: both widths report the same failure" {
    const Case = struct {
        document: []const u8,
        failure: anyerror,
    };
...
test "zix behaviour: a walk over one document reaches the same place at both widths" {
