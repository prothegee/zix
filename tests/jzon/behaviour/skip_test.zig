//! Behaviour tests: zix.jzon.skip, stepping over a whole value.
...
test "zix behaviour: skip steps over each scalar form" {
...
test "zix behaviour: skip steps over an object and everything inside it" {
...
test "zix behaviour: skip steps over an array and everything inside it" {
    try expectLeaves("[]|rest", "|rest");
...
test "zix behaviour: skip steps over a value laid out with whitespace" {
    try expectLeaves("  {  \"a\" :  [ 1 , 2 ]  ,  \"b\" :  true  }  |rest", "  |rest");
    try expectLeaves("\n\t\"padded\"\n|rest", "\n|rest");
}

test "zix behaviour: skip steps over a string carrying escapes and braces" {
...
test "zix behaviour: skip leaves a value it has already stepped past unread" {
    inline for (SHAPES) |shape| {
