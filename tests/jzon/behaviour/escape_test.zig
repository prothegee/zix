//! Behaviour tests: zix.jzon.escape, the string rules both directions share.
...
test "zix behaviour: escape encode matches std.json.Stringify byte for byte" {
    for (SAMPLES) |sample| {
...
test "zix behaviour: escape encode wraps the body in quotes" {
...
test "zix behaviour: escape decode returns every sample unchanged" {
    for (SAMPLES) |sample| {
...
test "zix behaviour: escape decode agrees with what std.json parses" {
    for (SAMPLES) |sample| {
        var encoded_buf: [256]u8 = undefined;
...
test "zix behaviour: needsEscape answers true for exactly what encode rewrites" {
    for (SAMPLES) |sample| {
        var encoded_buf: [256]u8 = undefined;
        var encoded: Sink = .init(&encoded_buf);
        try escape.encodeBody(&encoded, sample);
...
test "zix behaviour: escape decode reads the solidus form encode never writes" {
...
test "zix behaviour: escape decode turns a surrogate pair into one code point" {
    var buf: [32]u8 = undefined;
    var sink: Sink = .init(&buf);
