//! Behaviour tests: zix.jzon.string_value, the bytes a parse hands back for one
...
test "zix behaviour: a borrowed clean token is the document's own bytes" {
...
test "zix behaviour: a copied clean token stops depending on the document" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix behaviour: copy is what a parse does unless it is told otherwise" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix behaviour: an escaped token is decoded whichever mode is asked for" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix behaviour: every escape the rules spell is read back" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix behaviour: a decoded token is exactly as long as its value" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
