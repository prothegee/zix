...
const DeserializeStrategy = zix.jzon.DeserializeStrategy;
const SerializeStrategy = zix.jzon.SerializeStrategy;
const Strings = zix.jzon.Strings;
...
test "zix behaviour: every strategy pair round trips an order" {
    try expectEveryPair(ORDER);
}

test "zix behaviour: every strategy pair round trips an order with an unset optional" {
    var order = ORDER;
    order.note = null;
...
test "zix behaviour: every strategy pair round trips an order carrying escapes" {
    var order = ORDER;
...
test "zix behaviour: every strategy pair round trips an order with empty slices" {
    var order = ORDER;
    order.tags = &.{};
    order.lines = &.{};
...
test "zix behaviour: every strategy pair round trips the extremes of the number widths" {
    var order = ORDER;
...
test "zix behaviour: every serialize strategy renders the same bytes" {
    var expected_buf: [2048]u8 = undefined;
    const expected_len = try zix.jzon.serialize(&expected_buf, ORDER, .{});

    inline for (WRITE) |strategy| {
        var buf: [2048]u8 = undefined;
        const len = try zix.jzon.serialize(&buf, ORDER, .{ .strategy = strategy });
...
fn expectEveryReadStrategy(expected: Order, rendered: []const u8) !void {
    ...
    inline for (READ) |strategy| {
        inline for (MODES) |mode| {
            const actual = try zix.jzon.deserialize(Order, arena.allocator(), rendered, .{
                .strategy = strategy,
                .strings = mode,
            });
