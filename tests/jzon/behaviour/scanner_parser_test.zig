//! Behaviour tests: zix.jzon.scanner_parser, std's tokens with generated dispatch.
...
test "zix behaviour: scanner parser reads a whole order" {
...
test "zix behaviour: scanner parser and the std path read the same order" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix behaviour: scanner parser reads a whitespace-heavy document the same way" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix behaviour: scanner parser reads an unset optional as null" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix behaviour: scanner parser gives an absent field the default its type declares" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix behaviour: scanner parser borrows a clean string and copies an escaped one" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix behaviour: scanner parser copies every string when asked to" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix behaviour: scanner parser refuses a key the type does not declare" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix behaviour: scanner parser steps over a key the type does not declare when asked to" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix behaviour: scanner parser reads a bare value as well as an object" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
...
test "zix behaviour: scanner parser reads a field the document puts out of order" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
